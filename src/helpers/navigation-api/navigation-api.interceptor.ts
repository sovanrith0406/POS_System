...
export class NavigationApiInterceptor implements HttpInterceptor
{
    /**
     * Constructor
     */
    constructor(
        @Inject(NAVIGATION_API_DEFAULT_DELAY) private _defaultDelay: number,
        private _navigationApiService: NavigationApiService
    )
    {
    }
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>>
    {
        ...
        const {
                  handler,
                  urlParams
              } = this._navigationApiService.findHandler(request.method.toUpperCase(), request.url);
